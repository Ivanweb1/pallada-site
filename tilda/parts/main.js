/* Вёрстка «Паллады» внутри блока Тильды.
   ...
   в блоке он заводит свой контейнер .pallada, в HEAD — все блоки страницы. */
(function () {
  function init(SCOPE) {
  ...
  (function () {
    ...
    /* ---- Мобильное меню ---- */
    var burger = SCOPE.querySelector('.js-burger');
    var headerEl = SCOPE.querySelector('.header');

    if (burger && headerEl) {
      var menu = PL_byId(burger.getAttribute('aria-controls'));
    ...
    function initCarousel(trackSel, prevSel, nextSel) {
      var track = SCOPE.querySelector(trackSel);
      var prev = SCOPE.querySelector(prevSel);
      var next = SCOPE.querySelector(nextSel);
    ...
    function initDots(dotsSel, viewportSel, trackSel) {
      var dots = SCOPE.querySelector(dotsSel);
      var viewport = SCOPE.querySelector(viewportSel);
      var track = SCOPE.querySelector(trackSel);
    ...
    var accItems = SCOPE.querySelectorAll('.js-acc-item');
    ...
    Array.prototype.forEach.call(SCOPE.querySelectorAll('.js-slider'), function (slider) {
    ...
    Array.prototype.forEach.call(SCOPE.querySelectorAll('.prj-stages__panels'), function (panels) {
    ...
    var stageTabs = SCOPE.querySelector('.js-stages-tabs');
    var stagePanels = SCOPE.querySelector('.js-stages-panels');
    ...
    var bioOpeners = SCOPE.querySelectorAll('.js-bio-open');
    if (bioOpeners.length) {
      ...
      Array.prototype.forEach.call(bioOpeners, function (btn) {
        var modal = PL_byId(btn.getAttribute('aria-controls'));
        if (!modal) return;
      ...
      document.addEventListener('keydown', function (e) {
        ...
        var open = SCOPE.querySelector('.modal:not([hidden])');
        if (open) bioClose(open);
      });
    }

    /* ---- Уведомление о cookie ---- */
    var cookieBar = SCOPE.querySelector('.js-cookie');
    ...
    Array.prototype.forEach.call(SCOPE.querySelectorAll('.js-cform input[type="tel"]'), function (input) {
    ...
    Array.prototype.forEach.call(SCOPE.querySelectorAll('.cform__agree a'), function (link) {
    ...
    /* ---- Фильтры портфолио ---- */
    var filters = SCOPE.querySelectorAll('.js-pf-filter');
    var pfGrid = SCOPE.querySelector('.js-pf-grid');
    var pfEmpty = SCOPE.querySelector('.js-pf-empty');
  ...
  function initAll() {
    var blocks = document.querySelectorAll('.pallada');
    for (var i = 0; i < blocks.length; i++) init(blocks[i]);
  }

  /* скрипт лежит внутри блока — заводим только его */
  var own = document.currentScript && document.currentScript.closest('.pallada');
